    ...
    <div className="w-full rounded-2xl overflow-hidden border border-border bg-surface shadow-md mt-2">
      ...
      <div className="divide-y divide-border">
        ...
        <div className="flex items-center px-4 py-2">
          <span className="text-[13px] text-text-secondary w-[50px] shrink-0">To</span>
          {isEditing ? (
            <input
              className="flex-1 text-[13px] text-text-primary outline-none bg-transparent"
              value={to}
              onChange={(e) => setEditPayload({ ...editPayload, to: e.target.value })}
              placeholder="Recipients"
            />
          ) : (
            <span className="flex-1 text-[13px] text-text-primary font-medium">{to}</span>
          )}
        </div>

        {/* Subject field */}
        <div className="flex items-center px-4 py-2">
          <span className="text-[13px] text-text-secondary w-[50px] shrink-0">Subject</span>
          {isEditing ? (
            <input
              className="flex-1 text-[13px] text-text-primary outline-none bg-transparent"
              value={subject}
            ...
            <span className="flex-1 text-[13px] text-text-primary font-medium">{subject}</span>
          )}
        </div>
        ...
        <div className="px-4 py-3 min-h-[120px]">
          {isEditing ? (
            <textarea
              className="w-full text-[13px] text-text-primary outline-none bg-transparent min-h-[120px] resize-y leading-relaxed"
              value={body}
              onChange={(e) => setEditPayload({ ...editPayload, body: e.target.value })}
              placeholder="Compose email"
            />
          ) : (
            <div className="text-[13px] text-text-primary whitespace-pre-wrap leading-relaxed">{body}</div>
          )}
        </div>
      </div>
    </div>
