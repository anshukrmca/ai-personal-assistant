  ...
  return (
    <>
      ...
      <div className={`mt-2 bg-surface-raised p-3 border border-border-soft rounded-lg text-text-secondary whitespace-pre-wrap font-medium ${editingActionId === m.id ? 'focus-within:border-accent/50' : ''}`}>
        {editingActionId === m.id ? <textarea className="w-full bg-transparent border-none rounded outline-none min-h-[80px] resize-y text-text-primary" value={editPayload.body || ''} onChange={e => setEditPayload({...editPayload, body: e.target.value})} /> : (m.action.payload as any).body}
      </div>
    </>
  );
}
