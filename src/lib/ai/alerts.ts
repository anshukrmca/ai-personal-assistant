...
export async function draftAlertResponse(item: FeedItem): Promise<string> {
  const getMockDraft = async () => {
    await new Promise((r) => setTimeout(r, 1200));
    return `Hi, I'm following up on "${item.title}". Let's coordinate on this right away. Best, Anshu.`;
  };
  ...
  try {
    ...
    console.warn("[aiService] draftAlertResponse failed, falling back to mock:", err);
...
export async function suggestAlertRules(items: FeedItem[]): Promise<Array<{ id: string, title: string, desc: string, active: boolean }>> {
  ...
  try {
    ...
    console.warn("[aiService] suggestAlertRules failed, falling back to mock:", err);
    return getMockRules();
  }
}
