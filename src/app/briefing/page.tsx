...
export default function BriefingHubPage() {
  ...
  return (
    <AppShell>
      <div className="px-4 md:px-10 py-12 flex flex-col min-h-screen relative overflow-hidden">
        ...
        <div className="mb-14 relative z-10 mt-2 flex flex-col lg:flex-row items-center justify-between gap-8 md:gap-10 bg-surface/50 border border-border/60 rounded-[2rem] p-5 sm:p-8 md:p-10 shadow-sm backdrop-blur-md overflow-hidden">
          ...
          <div className="flex-1 max-w-2xl relative z-10">
            <div className="inline-flex items-center justify-center px-4 py-1.5 sm:py-2 mb-4 sm:mb-6 rounded-full bg-accent/10 text-accent border border-accent/20 shadow-sm">
              <span className="font-bold tracking-wide uppercase text-[10px] md:text-[11px]">Your Command Center</span>
            </div>
            <h1 className="font-display font-extrabold text-[10.5px] md:text-[28px] sm:text-4xl md:text-5xl tracking-tight text-text-primary mb-3 sm:mb-5 leading-tight">
              Briefing <span className="text-transparent bg-clip-text bg-gradient-to-r from-accent to-[#9061f9]">Hub</span>
            </h1>
            <p className="text-text-secondary text-[10.5px] md:text-[14px] md:text-lg leading-relaxed mb-0">
          ...
          <div className="shrink-0 w-full lg:w-auto relative z-10">
            ...
            <div className="bg-surface border border-border/60 rounded-[1.5rem] p-1 shadow-sm w-full min-w-0 sm:min-w-[280px] relative group">
               ...
               <div className="relative bg-surface rounded-[1.3rem] p-6 h-full text-center">
                 <h3 className="text-[10px] md:text-[12px] font-bold text-text-tertiary uppercase tracking-wider mb-3">Today's Briefing</h3>
                 <div className="font-display font-bold text-2xl sm:text-3xl text-text-primary mb-1">
                   {new Date().toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })}
                 </div>
                 <div className="text-[10px] md:text-[12px] sm:text-[10.5px] md:text-[14px] font-bold text-accent">
                   {new Date().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}
                 </div>
                 
                 <div className="mt-5 pt-5 border-t border-border/50 flex items-center justify-center gap-6">
                    <div className="text-center">
                      <div className="text-xl sm:text-2xl font-display font-bold text-text-primary">{items.filter(i => i.priority === "high" || i.requiresFollowUp).length}</div>
                    ...
                    <div className="text-center">
                      <div className="text-xl sm:text-2xl font-display font-bold text-text-primary">{items.length}</div>
            ...
            {connectedPlatforms.map((platformKey) => {
              ...
              return (
                <button
                  ...
                  className="group relative overflow-hidden bg-surface border border-border/60 rounded-[2rem] p-6 sm:p-8 hover:shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:border-accent/30 hover:-translate-y-1 transition-all duration-300 text-left flex flex-col h-full min-h-[260px]"
                >
                  ...
                  <h3 className="font-display font-bold text-lg sm:text-xl text-text-primary mb-3 flex items-center justify-between z-10">
                    {p.label}
                  </h3>
                  
                  <p className="text-text-secondary text-[10.5px] md:text-[10.5px] md:text-[14.5px] leading-relaxed mb-8 flex-grow z-10">
                    {p.desc}
                  </p>
                  
                  <div className="flex items-center justify-between w-full pt-5 border-t border-border/50 z-10">
                    <div className="flex items-center gap-2.5">
                      <span className="flex items-center justify-center min-w-[28px] h-7 px-2.5 rounded-full font-bold text-[10.5px] md:text-[10.5px] md:text-[13px] shadow-sm border transition-colors"
                        style={{ 
                      ...
                      <span className="text-[10.5px] md:text-[10.5px] md:text-[13px] font-semibold text-text-secondary">
        ...
        {!loading && connectedPlatforms.length > 0 && (
          <div className="mt-16 grid grid-cols-1 lg:grid-cols-3 gap-8 relative z-10 max-w-6xl mx-auto w-full">
            ...
            <div className="lg:col-span-2">
              <h2 className="font-display font-bold text-lg sm:text-xl text-text-primary mb-6 flex items-center gap-3">
                <span className="w-1.5 h-6 bg-danger rounded-full"></span>
                Needs Attention
              </h2>
              <div className="space-y-4">
                {items.filter(i => i.priority === "high" || i.requiresFollowUp).length > 0 ? (
                  items.filter(i => i.priority === "high" || i.requiresFollowUp).slice(0, 4).map((item, index) => {
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0 pt-0.5">
                          <div className="flex items-center justify-between mb-1.5">
                            ...
                            <span className="text-[10px] md:text-[10px] md:text-[12px] text-text-tertiary font-semibold">
                              {new Date(item.receivedAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                            </span>
                          </div>
                          <h4 className="font-bold text-text-primary text-[10.5px] md:text-[13px] md:text-[10.5px] md:text-[15px] mb-1 truncate">{item.title}</h4>
                          <p className="text-text-secondary text-[10.5px] md:text-[10.5px] md:text-[14px] line-clamp-1">{item.snippet}</p>
                        </div>
                      </div>
                    );
                  })
                  ...
                  <div className="bg-surface border border-border/60 rounded-[1.5rem] p-8 text-center shadow-sm">
                    ...
                    <h3 className="font-bold text-text-primary text-base md:text-lg">All caught up!</h3>
                    <p className="text-text-secondary text-[10.5px] md:text-[10.5px] md:text-[14px] mt-1">No urgent alerts requiring your attention right now.</p>
                  </div>
                )}
              </div>
            </div>
                ...
                <div className="space-y-6">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4 text-text-secondary">
                      ...
                      <span className="font-semibold text-[10.5px] md:text-[10.5px] md:text-[14px]">Inbox Emails</span>
                    </div>
                    <span className="font-display font-bold text-lg sm:text-xl text-text-primary">{getPlatformCount('gmail')}</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4 text-text-secondary">
                      ...
                      <span className="font-semibold text-[10.5px] md:text-[10.5px] md:text-[14px]">Meetings</span>
                    </div>
                    <span className="font-display font-bold text-lg sm:text-xl text-text-primary">{getPlatformCount('google_calendar')}</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4 text-text-secondary">
                      ...
                      <span className="font-semibold text-[10.5px] md:text-[10.5px] md:text-[14px]">Messages</span>
                    </div>
                    <span className="font-display font-bold text-lg sm:text-xl text-text-primary">{getPlatformCount('whatsapp') + getPlatformCount('slack')}</span>
                  </div>
                </div>
                
                <button 
                  onClick={() => router.push('/dashboard')}
                  className="w-full mt-8 py-3 bg-surface hover:bg-surface-raised border border-border/60 text-text-primary font-bold rounded-xl transition-colors text-[10.5px] md:text-[10.5px] md:text-[13px] shadow-sm"
                >
                  View Full Dashboard
                </button>
              </div>
